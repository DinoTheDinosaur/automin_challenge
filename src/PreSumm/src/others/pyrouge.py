...
class Rouge155(object):
    ...
    # The models filename pattern has '#ID#' as a placeholder for the
    # document ID. If there are multiple models summaries, pyrouge
    # will use the provided regex to automatically match them with
    # the corresponding system summary. Here, [A-Z] matches
    # multiple models summaries for a given #ID#.
    ...
    def model_filename_pattern(self):
        """
        The regular expression pattern for matching models summary
        filenames. The pattern needs to contain the string "#ID#",
        which is a placeholder for the document ID.

        E.g. "SL.P.10.R.[A-Z].SL062003-#ID#.html" will match the models
        ...
        The different models summaries for a given document ID are
...
{elems}
    ...
    def write_config_static(system_dir, system_filename_pattern,
        ...
        of system summary files and their corresponding models summary
        files.

        pyrouge uses regular expressions to automatically find the
        matching models summary files for a given system summary file
        ...
        model_filename_pattern).
            ...
            model_dir:                  Path of directory containing
                                        models summaries.
            model_filename_pattern:     Regex string for matching models
                                        summary filenames.
    ...
    def write_config(self, config_file_path=None, system_id=None):
        """
        Write the ROUGE configuration file, which is basically a list
        of system summary files and their matching models summary files.
    ...
    def evaluate(self, system_id=1, rouge_args=None):
        ...
        the models summaries in model_dir. The summaries are assumed to
    ...
    def convert_and_evaluate(self, system_id=1,
        ...
        evaluate the system summaries in system_dir against the models
    ...
    def __get_eval_string(
        ...
        against several models summaries, i.e. there is an m-to-n
        relation between system and models summaries. The system
        summaries are listed in the <PEERS> tag and the models summaries
        in the <MODELS> tag. pyrouge currently only supports one system
        summary per text, i.e. it assumes a 1-to-n relation between
        system and models summaries.
...
""".format(
    ...
    def __process_summaries(self, process_func):
        ...
        system and models folders and saves the resulting files to new
        system and models folders.

        """
        temp_dir = mkdtemp(dir=self.temp_dir)
        new_system_dir = os.path.join(temp_dir, "system")
        os.mkdir(new_system_dir)
        new_model_dir = os.path.join(temp_dir, "models")
        os.mkdir(new_model_dir)
        self.log.info(
            "Processing summaries. Saving system files to {} and "
            "models files to {}.".format(new_system_dir, new_model_dir))
    ...
    def __get_model_filenames_for_id(id, model_dir, model_filenames_pattern):
        ...
        if not model_filenames:
            raise Exception(
                "Could not find any models summaries for the system"
                " summary with ID {}. Specified models filename pattern was: "
    ...
    def __set_dir_properties(self):
        ...
        directories = [
            ...
            ("models", "Path of the directory containing models summaries."),
            ]
